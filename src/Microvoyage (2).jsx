...
function generateTrainTrips(region, days, budget, moods) {
  return TRAIN_TRIPS.filter(t => {
    const hasHub = Object.keys(t.trainH).some(h => h === region || region.startsWith(h.split(" ")[0]));
    const moodOk = moods.includes("all") || moods.some(m => t.moods.includes(m));
    const fromHub = Object.keys(t.trainH).find(h => h === region || region.startsWith(h.split(" ")[0]));
    const price = fromHub ? t.price[fromHub] * 2 : 999; // round trip estimate
    return hasHub && moodOk && price <= budget;
  }).map((t, i) => {
    const fromHub = Object.keys(t.trainH).find(h => h === region || region.startsWith(h.split(" ")[0]));
    const oneWayH = t.trainH[fromHub] || 2;
    const roundTripPrice = Math.round(t.price[fromHub] * 2 * 1.1); // slight markup for r/t
    const fromStation = AMTRAK_STATIONS[region] || AMTRAK_STATIONS[fromHub] || "WAS";
...
export default function App() {
  ...
  const search = () => {
    ...
    setTimeout(() => {
      ...
      const trains = generateTrainTrips(region, days, budget, moods);
      setTrips(result);
