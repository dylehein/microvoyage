...
const TRAIN_TRIPS = [
  // Northeast Corridor
  { city:"New York City", station:"NYP", stationName:"Penn Station", hubs:["Washington DC","Philadelphia","Boston"], trainH:{ "Washington DC":3.0, "Philadelphia":1.25, "Boston":3.5 }, price:{ "Washington DC":89, "Philadelphia":29, "Boston":110 }, moods:["city","food","culture","romance"], vibe:"City", flag:"🇺🇸", highlights:["Central Park","Chelsea food halls","Brooklyn Bridge walk"], hotel:[120,220,480] },
  { city:"Philadelphia", station:"PHL", stationName:"30th Street Station", hubs:["Washington DC","New York","Boston"], trainH:{ "Washington DC":1.75, "New York":1.25, "Boston":5.5 }, price:{ "Washington DC":49, "New York":29, "Boston":89 }, moods:["culture","food","history"], vibe:"History", flag:"🇺🇸", highlights:["Reading Terminal Market","Eastern State Penitentiary","Cheesesteak tour"], hotel:[90,160,320] },
  { city:"Washington DC", station:"WAS", stationName:"Union Station", hubs:["New York","Philadelphia","Boston"], trainH:{ "New York":3.0, "Philadelphia":1.75, "Boston":6.5 }, price:{ "New York":89, "Philadelphia":49, "Boston":120 }, moods:["culture","city","history"], vibe:"Culture", flag:"🇺🇸", highlights:["Smithsonian museums","Capitol Hill","Georgetown waterfront"], hotel:[100,180,400] },
  { city:"Boston", station:"BOS", stationName:"South Station", hubs:["New York","Washington DC","Philadelphia"], trainH:{ "New York":3.5, "Washington DC":6.5, "Philadelphia":5.5 }, price:{ "New York":110, "Washington DC":120, "Philadelphia":89 }, moods:["culture","food","history","cozy"], vibe:"History", flag:"🇺🇸", highlights:["Freedom Trail walk","Fenway Park","North End pasta & cannoli"], hotel:[110,190,420] },
  // Midwest
  { city:"Chicago", station:"CHI", stationName:"Union Station", hubs:["Minneapolis","Milwaukee","Detroit"], trainH:{ "Minneapolis":7.5, "Milwaukee":1.5, "Detroit":5.5 }, price:{ "Minneapolis":49, "Milwaukee":25, "Detroit":39 }, moods:["city","food","culture"], vibe:"City", flag:"🇺🇸", highlights:["Architecture river cruise","Deep dish pizza","Art Institute"], hotel:[110,195,420] },
...
const AMTRAK_STATIONS = {
  ...
  "Chicago": "CHI",
  "Los Angeles": "LAX",
  "San Francisco": "OKJ", // Oakland Jack London
  "Seattle": "SEA",
  "Minneapolis": "MSP",
};
...
function generateTrainTrips(region, days, budget, moods) {
  return TRAIN_TRIPS.filter(t => {
    ...
    const moodOk = moods.includes("all") || moods.some(m => t.moods.includes(m)) || t.moods.includes("history");
          ...
          <div style={{ display:"flex", gap:16, flexWrap:"wrap", marginBottom:20 }}>
            ...
            <div style={{ flex:"1 1 160px", minWidth:0 }}>
              <label style={{ display:"block", fontSize:10, letterSpacing:"0.22em", textTransform:"uppercase", color:G, marginBottom:8 }}>
                ...
                {party > 1 && budgetMode === "pp" && <span style={{ color:"#666", marginLeft:4 }}>(${(budget * party).toLocaleString()} total)</span>}
                {party > 1 && budgetMode === "total" && <span style={{ color:"#666", marginLeft:4 }}>(${Math.round(budget / party).toLocaleString()}/pp)</span>}
              </label>
              {party > 1 && (
                <div style={{ display:"flex", gap:4, marginBottom:8 }}>
                  {[["pp","Per Person"],["total","Total Trip"]].map(([val,lbl]) => (
                    <button key={val} onClick={() => setBudgetMode(val)}
                      style={{ flex:1, padding:"4px 0", borderRadius:6, border: budgetMode===val ? `1px solid ${G}` : "1px solid rgba(255,255,255,0.1)", background: budgetMode===val ? GL : "transparent", color: budgetMode===val ? G : "#555", cursor:"pointer", fontSize:10, fontFamily:"Georgia,serif", transition:"all 0.2s" }}>
                      {lbl}
                    </button>
                  ))}
                </div>
                        ...
                        {[{label:"🛫 Outbound",f:trip.outbound,url:gfOut},{label:"🛬 Return",f:trip.return,url:gfRet}].map(({label,f,url}) => (
                          <div key={label} style={{ flex:"1 1 190px", minWidth:0, background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, padding:12 }}>
                            ...
                            <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:"#555", marginBottom:8 }}>
                              ...
                              <span style={{ color:"#7fc47f" }}>~${f.estimated_price} <span style={{color:"#444",fontSize:9}}>est.</span></span>
                            </div>
                            <div style={{ fontSize:9, color:"#444", marginBottom:6, fontStyle:"italic" }}>
                              Times & prices are typical estimates — tap below for live fares
                            </div>
        ...
        {trainTrips && trainTrips.length > 0 && (
          <div style={{ marginTop:24 }}>
            <button onClick={() => setShowTrains(p => !p)}
              style={{ width:"100%", padding:"13px 17px", background:"rgba(100,180,255,0.04)", border:"1px solid rgba(100,180,255,0.15)", borderRadius:12, color:"#8ab4f8", cursor:"pointer", fontFamily:"Georgia,serif", fontSize:13, letterSpacing:"0.08em", display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom: showTrains ? 10 : 0 }}>
            ...
            {showTrains && trainTrips.map((trip, idx) => {
              ...
              return (
                <div key={trip.id} style={{ background:"rgba(100,180,255,0.03)", border: exp ? "1px solid rgba(100,180,255,0.3)" : "1px solid rgba(100,180,255,0.1)", borderRadius:13, overflow:"hidden", marginBottom:8, transition:"border 0.3s" }}>
                  <div style={{ padding:"14px 17px", cursor:"pointer", display:"flex", alignItems:"center", gap:11 }} onClick={() => setExpanded(exp ? null : trip.id)}>
                    <div style={{ fontSize:20, flexShrink:0 }}>🚂</div>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap", marginBottom:3 }}>
                        <span style={{ fontSize:15 }}>{trip.flag}</span>
                        <span style={{ fontSize:16 }}>{trip.destination_city}</span>
                        <span style={{ fontSize:9, padding:"2px 7px", borderRadius:20, background:"rgba(100,180,255,0.1)", color:"#8ab4f8", border:"1px solid rgba(100,180,255,0.25)", letterSpacing:"0.08em", textTransform:"uppercase" }}>Train</span>
                        <span style={{ fontSize:9, padding:"2px 7px", borderRadius:20, background:GOLD_LIGHT, color:GOLD, border:`1px solid ${GOLD_BORDER}`, letterSpacing:"0.08em", textTransform:"uppercase" }}>{trip.vibe}</span>
                      </div>
                      <div style={{ fontSize:11, color:"#444", fontStyle:"italic" }}>
                        {trip.fromHub} → {trip.destination_city} · {trip.one_way_hours}h each way · via Amtrak
                      </div>
                    </div>
                    <div style={{ display:"flex", gap:12, alignItems:"center", flexShrink:0 }}>
                      <div style={{ textAlign:"center" }}>
                        <div style={{ fontSize:17, color:"#7fc47f" }}>${trip.total_price}</div>
                        <div style={{ fontSize:8, color:"#444", textTransform:"uppercase", letterSpacing:"0.1em" }}>r/t est.</div>
                      </div>
                      <div style={{ textAlign:"center" }}>
                        <div style={{ fontSize:13, color:"#8ab4f8" }}>{Math.round(trip.hours_on_ground)}h</div>
                  ...
                  {exp && (
                    <div style={{ borderTop:"1px solid rgba(100,180,255,0.08)", padding:"14px 17px" }}>
                      <div style={{ background:"rgba(100,180,255,0.05)", border:"1px solid rgba(100,180,255,0.12)", borderRadius:10, padding:"12px 14px", marginBottom:12 }}>
                        <div style={{ fontSize:9, letterSpacing:"0.18em", textTransform:"uppercase", color:"#8ab4f8", marginBottom:8 }}>Journey</div>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                          <div><div style={{ fontSize:15 }}>{trip.fromHub}</div><div style={{ fontSize:10, color:"#555" }}>Your city</div></div>
                          <div style={{ textAlign:"center", color:"#333", fontSize:11 }}><div>{trip.one_way_hours}h</div><div style={{ margin:"3px 0" }}>──🚂──</div></div>
                          <div style={{ textAlign:"right" }}><div style={{ fontSize:15 }}>{trip.destination_city}</div><div style={{ fontSize:10, color:"#555" }}>{trip.station_name}</div></div>
                        </div>
                        <div style={{ fontSize:11, color:"#555", marginBottom:10 }}>No airport security · Walk-on baggage · Scenic views · City-center arrival</div>
                        <a href={amtrakUrl} target="_blank" rel="noopener noreferrer"
                          style={{ display:"block", textAlign:"center", padding:"9px", borderRadius:8, background:"rgba(100,180,255,0.08)", border:"1px solid rgba(100,180,255,0.2)", color:"#8ab4f8", fontSize:11, textDecoration:"none", letterSpacing:"0.12em", textTransform:"uppercase", fontWeight:600 }}>
                      ...
                      {trip.nights > 0 && trip.hotel && (
                        <div style={{ marginBottom:12 }}>
                          <div style={{ fontSize:9, letterSpacing:"0.18em", textTransform:"uppercase", color:"#444", marginBottom:7 }}>Hotel Estimates · {trip.nights} night{trip.nights > 1 ? "s" : ""}</div>
                          <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:8 }}>
                            {[{label:"Budget",key:"budget",color:"#7fc47f",icon:"🏨"},{label:"Mid-range",key:"mid",color:GOLD,icon:"🏩"},{label:"Luxury",key:"luxury",color:"#b89cf5",icon:"🏰"}].map(tier => (
                              <div key={tier.key} style={{ flex:"1 1 90px", minWidth:0, background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, padding:"9px 10px", textAlign:"center" }}>
                                <div style={{ fontSize:15, marginBottom:3 }}>{tier.icon}</div>
                                <div style={{ fontSize:9, color:"#555", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:3 }}>{tier.label}</div>
                                <div style={{ fontSize:17, color:tier.color }}>${trip.hotel[tier.key].toLocaleString()}</div>
                              </div>
                            ))}
                          </div>
                          <a href={"https://www.booking.com/searchresults.html?ss=" + encodeURIComponent(trip.destination_city) + "&checkin=" + startDate + "&checkout=" + (() => { const d = new Date(startDate + "T12:00:00"); d.setDate(d.getDate() + trip.nights); return d.toISOString().split("T")[0]; })()} target="_blank" rel="noopener noreferrer"
                            style={{ display:"block", textAlign:"center", padding:"7px", borderRadius:7, background:"rgba(127,196,127,0.08)", border:"1px solid rgba(127,196,127,0.2)", color:"#7fc47f", fontSize:10, textDecoration:"none", letterSpacing:"0.1em", textTransform:"uppercase" }}>
                            Search Booking.com ↗
                          </a>
                        </div>
                      ...
                      <div style={{ fontSize:10, color:"#444", fontStyle:"italic", marginTop:6 }}>
